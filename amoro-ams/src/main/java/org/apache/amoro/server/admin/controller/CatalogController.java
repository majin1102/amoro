...
import org.apache.amoro.admin.model.CatalogRegisterInfo;
import org.apache.amoro.admin.model.CatalogSettingInfo;
import org.apache.amoro.admin.model.CatalogSettingInfo.ConfigFileItem;
import org.apache.amoro.api.CatalogMeta;
import org.apache.amoro.catalog.UnifiedCatalogUtil;
import org.apache.amoro.properties.CatalogMetaProperties;
import org.apache.amoro.properties.S3Properties;
...
import org.apache.amoro.server.admin.utils.DesensitizationUtil;
import org.apache.amoro.server.admin.utils.PropertiesUtil;
...
public class CatalogController {
  ...
  static {
    ...
    // TODO remove iceberg dependency
  ...
  public CatalogController(
      CatalogService catalogService, PlatformFileManager platformFileInfoService) {
  ...
  private static Set<String> getHiddenCatalogProperties(
    ...
    if (AUTH_CONFIGS_VALUE_TYPE_AK_SK.equalsIgnoreCase(
      ...
      hiddenProperties.add(S3Properties.ACCESS_KEY_ID);
      hiddenProperties.add(S3Properties.SECRET_ACCESS_KEY);
    }
    if (STORAGE_CONFIGS_VALUE_TYPE_S3.equals(storageConfig.get(STORAGE_CONFIGS_KEY_TYPE))) {
      ...
      hiddenProperties.add(S3Properties.ENDPOINT);
    }
  ...
  private void fillAuthConfigs2CatalogMeta(
    ...
    switch (authType) {
      ...
      case AUTH_CONFIGS_VALUE_TYPE_AK_SK:
        ...
        UnifiedCatalogUtil.copyProperty(
            serverAuthConfig,
            catalogMeta.getCatalogProperties(),
            AUTH_CONFIGS_KEY_ACCESS_KEY,
            S3Properties.ACCESS_KEY_ID);
        UnifiedCatalogUtil.copyProperty(
            serverAuthConfig,
            catalogMeta.getCatalogProperties(),
            AUTH_CONFIGS_KEY_SECRET_KEY,
            S3Properties.SECRET_ACCESS_KEY);
        break;
    }
  ...
  private Map<String, Object> extractStorageConfigsFromCatalogMeta(
    ...
    String storageType = UnifiedCatalogUtil.getCompatibleStorageType(config);
    ...
    if (newInfo.getAuthConfig().containsKey(AUTH_CONFIGS_KEY_SECRET_KEY)) {
      ...
      if (DesensitizationUtil.isDesensitized(secretKey)) {
        Preconditions.checkArgument(
            oldCatalogMeta.getCatalogProperties().containsKey(S3Properties.SECRET_ACCESS_KEY),
            "Secret key is not set before，must provide a valid secret key");
        newInfo
            .getAuthConfig()
            .put(
                AUTH_CONFIGS_KEY_SECRET_KEY,
                oldCatalogMeta.getCatalogProperties().get(S3Properties.SECRET_ACCESS_KEY));
      }
    }
  }
  ...
  public void catalogDeleteCheck(Context ctx) {
    ...
    catalogService.getServerCatalog(catalogName);
