...
import org.apache.amoro.admin.model.LatestSessionInfo;
import org.apache.amoro.admin.model.LogInfo;
import org.apache.amoro.admin.model.SqlResult;
...
public class TerminalManager {
  ...
  private static final String ICEBERG_CATALOG_IMPL =
      "catalog-impl"; // pulled from Iceberg CatalogMeta
  ...
  public TerminalManager(Configurations conf, CatalogService catalogService) {
  ...
  public String executeScript(String terminalId, String catalog, String script) {
    CatalogMeta catalogMeta = catalogService.getCatalogMeta(catalog);
  ...
  private String catalogConnectorType(CatalogMeta catalogMeta) {
    ...
    Set<TableFormat> tableFormatSet = UnifiedCatalogUtil.tableFormats(catalogMeta);
  ...
  private TableMetaStore getCatalogTableMetaStore(CatalogMeta catalogMeta) {
    ...
    if (catalogMeta.getStorageConfigs() != null) {
      ...
      if (CatalogMetaProperties.STORAGE_CONFIGS_VALUE_TYPE_HADOOP.equalsIgnoreCase(
          UnifiedCatalogUtil.getCompatibleStorageType(storageConfigs))) {
        builder
  ...
  private void applyClientProperties(CatalogMeta catalogMeta) {
    Set<TableFormat> formats = UnifiedCatalogUtil.tableFormats(catalogMeta);
    String catalogType = catalogMeta.getCatalogType();
    if (formats.contains(TableFormat.ICEBERG)) {
      ...
      } else if (!catalogMeta.getCatalogProperties().containsKey(ICEBERG_CATALOG_IMPL)) {
