...
import org.apache.amoro.admin.model.FilesStatistics;
...
import org.apache.amoro.admin.model.TableBasicInfo;
import org.apache.amoro.admin.model.TableStatistics;
...
import org.apache.amoro.scheduler.persistence.OptimizingMapper;
import org.apache.amoro.server.admin.DDLReverser;
...
public class MixedAndIcebergAdmin extends PersistentBase implements TableFormatAdmin {

  private static final Logger LOG = LoggerFactory.getLogger(MixedAndIcebergAdmin.class);

  private final ExecutorService executorService;

  public MixedAndIcebergAdmin(ExecutorService executorService) {
    this.executorService = executorService;
  }

  @Override
  public List<TableFormat> getSupportedFormats() {
  ...
  public ServerTableMeta getTableDetail(AmoroTable<?> amoroTable) {
    ...
    baseMetrics.put("totalSize", AdminUtil.byteToXB(baseFilesStatistics.getTotalSize()));
    baseMetrics.put("fileCount", baseFilesStatistics.getFileCnt());
    baseMetrics.put("averageFileSize", AdminUtil.byteToXB(baseFilesStatistics.getAverageSize()));
    ...
    if (table.isKeyedTable()) {
      ...
      if (tableBasicInfo.getChangeStatistics() != null) {
        ...
        changeMetrics.put("totalSize", AdminUtil.byteToXB(changeFilesStatistics.getTotalSize()));
        changeMetrics.put("fileCount", changeFilesStatistics.getFileCnt());
        changeMetrics.put(
            "averageFileSize", AdminUtil.byteToXB(changeFilesStatistics.getAverageSize()));
        changeMetrics.put(
    ...
    tableSummary.put("size", AdminUtil.byteToXB(tableSize));
    tableSummary.put("file", tableFileCnt);
    tableSummary.put(
        "averageFile", AdminUtil.byteToXB(tableFileCnt == 0 ? 0 : tableSize / tableFileCnt));
          ...
          .forEach(
              snapshot -> {
                ...
                normalizeSummary.computeIfPresent(
                    ...
                    (k, v) -> AdminUtil.byteToXB(Long.parseLong(summary.get(k))));
                normalizeSummary.computeIfPresent(
            ...
            f ->
                result.add(
                    new PartitionFileBaseInfo(
                        commitId,
                        DataFileType.ofContentId(f.content().id()).toString(),
                        snapshotTime,
  ...
  public Pair<List<OptimizingProcessInfo>, Integer> getOptimizingProcessesInfo(
    ...
    return Pair.of(
        processMetaList.stream()
            .map(p -> IcebergAdminUtil.buildProcessInfo(p, optimizingTasks.get(p.getProcessId())))
            .collect(Collectors.toList()),
        total);
  }
  ...
  private TableBasicInfo getTableBasicInfo(MixedTable table) {
    try {
      ...
      if (table.isUnkeyedTable()) {
        ...
        IcebergTableStatCollector.fillTableStatistics(baseInfo, unkeyedTable, table);
      } else if (table.isKeyedTable()) {
        KeyedTable keyedTable = table.asKeyedTable();
        changeInfo = IcebergTableStatCollector.collectChangeTableInfo(keyedTable);
        baseInfo = IcebergTableStatCollector.collectBaseTableInfo(keyedTable);
      } else {
      ...
      tableBasicInfo.setTableStatistics(IcebergTableStatCollector.union(changeInfo, baseInfo));
  ...
  private ServerTableMeta getServerTableMeta(MixedTable table) {
    ...
    serverTableMeta.setPartitionColumnList(
        table.spec().fields().stream()
            .map(item -> IcebergAdminUtil.buildPartitionFieldFrom(table.spec().schema(), item))
            .collect(Collectors.toList()));
    serverTableMeta.setSchema(
        table.schema().columns().stream()
            .map(IcebergAdminUtil::buildColumnsInfoFrom)
            .collect(Collectors.toList()));
      ...
      if (kt.primaryKeySpec() != null) {
        serverTableMeta.setPkList(
            kt.primaryKeySpec().fields().stream()
                .map(item -> IcebergAdminUtil.buildColumnsInfoFrom(table.spec().schema(), item))
                .collect(Collectors.toList()));
      }
    }
            ...
            if (predicate.test(snapshotRef)) {
              result.add(
                  new TagOrBranchInfo(
                      ...
                      snapshotRef.isTag() ? TagOrBranchInfo.TAG : TagOrBranchInfo.BRANCH));
            }
          });
      result.sort(Comparator.comparing(TagOrBranchInfo::getName));
      putMainBranchFirst(result);
      return result;
    }
  }
