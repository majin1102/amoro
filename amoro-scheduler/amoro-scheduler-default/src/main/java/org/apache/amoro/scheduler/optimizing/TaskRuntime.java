...
import org.apache.amoro.scheduler.optimizing.plan.TaskDescriptor;
import org.apache.amoro.scheduler.persistence.OptimizingMapper;
import org.apache.amoro.scheduler.persistence.TaskFilesPersistence;
...
public class TaskRuntime extends StatedPersistentBase {
  ...
  @StateField private long startTime = DefaultSchedulerConstants.INVALID_TIME;
  @StateField private long endTime = DefaultSchedulerConstants.INVALID_TIME;
  @StateField private long costTime = DefaultSchedulerConstants.INVALID_TIME;
  ...
  void reset() {
    invokeConsistency(
        () -> {
          ...
          startTime = DefaultSchedulerConstants.INVALID_TIME;
          endTime = DefaultSchedulerConstants.INVALID_TIME;
          token = null;
          threadId = -1;
          failReason = null;
          output = null;
    ...
    invokeConsistency(
        () -> {
          if (statusMachine.tryAccepting(Status.CANCELED)) {
            endTime = System.currentTimeMillis();
            if (startTime != DefaultSchedulerConstants.INVALID_TIME) {
              costTime += endTime - startTime;
            }
            persistTaskRuntime(this);
          }
  ...
  public long getQuotaTime(long calculatingStartTime, long calculatingEndTime) {
    ...
    calculatingEndTime =
        costTime == DefaultSchedulerConstants.INVALID_TIME
            ? calculatingEndTime
            : costTime + startTime;
  ...
  private class TaskStatusMachine {
    ...
    public void accept(Status targetStatus) {
      ...
      if (!getNext().contains(targetStatus)) {
        throw new IllegalTaskStateException(taskId, status.toString(), targetStatus.toString());
      }
      status = targetStatus;
    }
