...
package org.apache.amoro.scheduler.maintainer;
...
import org.apache.amoro.scheduler.DataExpirationConfig;
...
public class MixedTableMaintainer implements TableMaintainer {
  ...
  public class BaseTableMaintainer extends IcebergTableMaintainer {
    ...
    public BaseTableMaintainer(UnkeyedTable unkeyedTable) {
      ...
      if (unkeyedTable.format() == TableFormat.MIXED_HIVE) {
        hiveFiles.addAll(getHiveLocation(mixedTable));
      }
    }
    ...
    private long fetchLatestOptimizedSequenceSnapshotTime(Table table) {
      if (mixedTable.isKeyedTable()) {
        Snapshot snapshot =
            findLatestSnapshotContainsKey(table, MixedTableUtil.BLOB_TYPE_OPTIMIZED_SEQUENCE_EXIST);
   ...
   */
  private static Set<String> getHiveLocation(MixedTable table) {
