...
import org.apache.amoro.properties.OptimizerProperties;
import org.apache.amoro.scheduler.executor.AsyncTableExecutors;
import org.apache.amoro.scheduler.table.RuntimeHandlerChain;
import org.apache.amoro.scheduler.table.TableService;
import org.apache.amoro.server.AmoroAdminService;
...
public class AmoroSchedulerContainer {

  public static final Logger LOG = LoggerFactory.getLogger(AmoroSchedulerContainer.class);
  ...
  public AmoroSchedulerContainer() throws Exception {
    initConfig();
  ...
  public static void main(String[] args) {
    try {
      AmoroSchedulerContainer service = new AmoroSchedulerContainer();
      while (true) {
        try {
  ...
  private void initHttpService() {
    AmoroAdminService adminService =
        new AmoroAdminService(serviceConfig, tableService, optimizingService, terminalManager);
    RestCatalogService restCatalogService = new RestCatalogService(tableService);

    httpServer =
        Javalin.create(
            config -> {
              config.addStaticFiles(adminService.configStaticFiles());
    ...
    httpServer.routes(
        () -> {
          adminService.endpoints().addEndpoints();
    ...
    httpServer.before(
        ctx -> {
          ...
          if (StringUtils.isNotEmpty(token)) {
            ...
            adminService.preHandleRequest(ctx);
          }
        });
    httpServer.exception(
        ...
        (e, ctx) -> {
          if (restCatalogService.needHandleException(ctx)) {
            ...
            adminService.handleException(e, ctx);
          }
        });
