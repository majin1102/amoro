...
package org.apache.amoro.scheduler.catalog;
...
import org.apache.amoro.scheduler.persistence.InternalTableMapper;
import org.apache.amoro.scheduler.persistence.TableBlockerMapper;
import org.apache.amoro.scheduler.table.InternalTableCreator;
import org.apache.amoro.scheduler.table.InternalTableHandler;
...
public abstract class InternalCatalog extends ServerCatalog {
  ...
  public List<String> listDatabases() {
    return getAs(
        InternalTableMapper.class,
        mapper -> mapper.selectDatabases(getMetadata().getCatalogName()));
  }
  ...
  public List<TableIDWithFormat> listTables(String database) {
    return getAs(
        ...
        .stream()
        .map(sid -> TableIDWithFormat.of(TableIdentifier.of(sid.getIdentifier()), sid.getFormat()))
  ...
  public TableMetadata createTable(TableMetadata tableMetadata) {
    validateTableIdentifier(tableMetadata.getTableIdentifier());
    ServerTableIdentifier tableIdentifier = tableMetadata.getTableIdentifier();
    doAsTransaction(
        () -> doAs(InternalTableMapper.class, mapper -> mapper.insertTable(tableIdentifier)),
        () -> doAs(InternalTableMapper.class, mapper -> mapper.insertTableMeta(tableMetadata)),
        () ->
            doAsExisted(
                CatalogMetaMapper.class,
        ...
        .orElseThrow(
            () ->
                new ObjectNotExistsException(
                    TableIdentifier.of(name(), database, table).toString()));
  }

  public List<TableMetadata> listTableMetadataInDatabase(String database) {
    return getAs(
        InternalTableMapper.class, mapper -> mapper.selectTableMetasByDb(name(), database));
  }
  ...
  public Integer getTableCount(String databaseName) {
    return getAs(InternalTableMapper.class, mapper -> mapper.selectTableCount(name()));
  }
  ...
  protected void validateTableIdentifier(ServerTableIdentifier tableIdentifier) {
